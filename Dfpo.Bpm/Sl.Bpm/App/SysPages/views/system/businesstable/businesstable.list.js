(function () {
    var controllerId = app.dialogs.define('businesstableList', '/App/SysPages/views/system/businesstable/businesstable.list.html');
    app.controller(controllerId, ['$scope', 'params', 'mabp.app.enterpriseInfoSync', '$stateParams',
        function ($scope, params, service, $stateParams) {
            ...
            vm.load = function () {
                service.getBusinessTableShowList(vm.paging).then(function (data) {
            ...
            vm.keyDown = function (event) {
                if (event.keyCode == 13) {
                    vm.paging.filters = [{ name:'SchemaName', value: vm.filterText }];
                    vm.load();
                }
            }
            ...
            vm.getSelected = function () {
                ...
                _.forEach(idsObj, function (value, key) {
                    ids.push(value.businessTableId);
                });
                if (ids.length > 0) {
                    mabp.ui.setSaving('getSelected',
                    service.getSelectedBusinessTables(ids).then(function (data) {
                        if (data) {
                            mabp.notify.success(L("SynchronizedSuccess"));
                            vm.load();
                            vm.isChanged = true;
                        }
                    }), L("GetChoosedDatatable"));
                }
            }

            vm.getAll = function () {
                mabp.ui.setSaving('getAll',
                service.getBusinessTables().then(function (data) {
                    if (data) {
                        mabp.notify.success(L("SynchronizedSuccess"));
                        vm.load();
                    }

...
function CheckPermission(permissionKey) {
    if (!window.permissions[permissionKey])
        window.location.href = window.location.origin;
}

//多语言合并
function MultiL(language) {
    if (!mabp.localization['Bpm.' + language]) {
        mabp.localization['Bpm.' + language] = {};
        var langs = language.split(",");
        _.forEach(langs, function (data) {
            for (var keyLocal in mabp.localization) {
                if (mabp.localization.hasOwnProperty(keyLocal)) {
                    if (keyLocal.toLowerCase() !== ("Bpm." + data).toLowerCase()) continue;
                    var localization = mabp.localization[keyLocal];
                    for (var key in localization) {
                        if (localization.hasOwnProperty(key)) {
                            if (!mabp.localization['Bpm.' + language][key]) {
                                mabp.localization['Bpm.' + language][key] = localization[key];
                            } else {
                                if (mabp.localization['Bpm.' + language][key].indexOf(localization[key]) < 0)
                                    mabp.localization['Bpm.' + language][key] = mabp.localization['Bpm.' + language][key] + "<br/>" + localization[key];
                            }
                        }
                    }
                }
    ...
    return mabp.localization['Bpm.' + language];
}
...
(function () {
    ...
    _shared.factory('appSession', ['mabp.app.system', 'mabp.app.permission',
        function (service, permissionService) {
            ...
            //将NameValue 给转化一下
            function getPermission(nameValueList) {
                var rtnData = {};
                _.forEach(nameValueList, function (item) {
                    rtnData[item.name] = item.value;
                });
                return rtnData;
            }
            permissionService.getUserPermissions().then(function (permissions) {
                window.permissions = session.permissions = getPermission(permissions);
...
//    _shared.config(['$provide', function($provide) {
...
//                        var jsExceptionMessage = "Js运行时异常. <br/><pre>" + exception.message +"</pre>";
//                        mabp.notify.error(jsExceptionMessage);
//                    };
//                }
//            ]);
//        }
//    ]);
    ...
    _shared.initialPage = function (vm, currentPage, pageSize, orderByProperty, ascending) {
        var pageParam = {
            ...
            pushFilter: function (i) {
                ...
                var obj = _.find(this.filters, { name: i.name });
                if (!!obj) obj.value = i.value;
                else {
                    this.filters.push({ name: i.name, value: i.value });
                }
            }
        }
        if (vm.paging != null)
    ...
    _shared.filter('trustHtml', function ($sce) {
        return function (input) {
            return $sce.trustAsHtml(input);
        }
    });

    _shared.directive('fmBind', function () {
        return {
            restrict: 'A',
            scope: {
                fmBind: '='
            ...
            link: function (scope, elem, attr) {
                elem.html(scope.fmBind);
            }

        }
    });

    _shared.escape2Html = function escape2Html(str) {
        var arrEntities = { 'lt': '<', 'gt': '>', 'nbsp': ' ', 'amp': '&', 'quot': '"' };
        return str.replace(/&(lt|gt|nbsp|amp|quot);/ig, function (all, t) { return arrEntities[t]; });
    }

    _shared.directive('ngEnter', function () {

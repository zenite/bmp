...
.directive("fmDatatextSelect", ['fmTool', '$timeout', 'dialog', 'mabp.app.module', 'mabp.app.bpm', function (fmTool, $timeout, dialog, service, bpmService) {
    return {
        ...
        template: function (elem, attr) {
            ...
            return '<div fm-model="' + attr.fmModel + '" fm-text-model="' + attr.fmTextModel + '" class="input-sm ma-datatext-select ma-data-select ' + (attr.ismulti ? 'multi' : 'single') + ' ' + attr.class + '" ng-class="state" ng-mousedown="openSelect($event)" ng-blur="event_Blur()" style="display: inline-block;">' +
                      (!!attr.placeholder ? '<div class="placeholder" ng-class="{\'hide\':!!fmModel}">' + attr.placeholder + '</div>' : '') +
                      '<div class="select-data-value">' +
                          '<input placeholder="' + attr.placeholder + '" class="' + (attr.href ? 'href-item' : '') + '" type="" ng-readonly="state==\'readonly\'" ng-disabled="state==\'disabled\' || ' + attr.textDisabled + '" ng-model="fmTextModel" ng-change="changeTxt()" />' +
                          '<div class="item"><span> {{ fmTextModel }}</span></div>' +
                      '</div>' +
        ...
        compile: function (tElement, tAttrs, transclude) {
            return {
                ...
                post: function postLink(scope, element, attr, cacheCtrl) {
                    ...
                    /* 若存在Id使用Id带出的Text 否则直接使用Text*/
                    //                    function setShadeText() {
                    //                        var shadeText = '';
                    //                        if (scope.fmModel != null) {
                    //
                    //                            var textArr = _.map(scope.selectItem, attr.displayMember);
                    //                            if (textArr != null) {
                    //                                shadeText = textArr.join(' ');
                    //                            } else {
                    //                                shadeText = '';
                    //                            }
                    //                        } else {
                    //                            shadeText = fmTextModel;
                    //                        }
                    //                        scope.shadeText = shadeText;
                    //                    }

    ...
    return {
        ...
        template: function (elem, attr) {
            ...
            return '<div field-name="{{fieldName}}" field-text="{{getShadeText()}}" class="input-sm fm-text" ng-class="state" ng-mousedown="event_textboxClick($event)" ng-keydown="event_textBoxKeyDown($event)">' +
                        '<input ' + _validate.join(' ') + ' placeholder="{{fmPlaceholder}}" class="fm-text-input validate" ng-readonly="state==\'readonly\'" ng-disabled="state==\'disabled\'" type="text" ng-model="fmModel" ng-change="event_changeTxt($event)" ng-focus="event_textFocus($event)" ng-blur="event_textBlur($event)" autocomplete="off" ma-dynamic-element />' +
        ...
        link: function (scope, element, attr) {
            ...
            scope.event_textBoxKeyDown = function (event) {
                ...
                //不修改fm-number的小数位，在这个地方做数量的限制，限制为数字和小数点
                if (attr.qty != undefined && attr.qty !== "false") {
                    //var userAgent = navigator.userAgent;
                    var isNum = false;
                    switch (event.key) {
                        case "0":
            ...
            scope.event_changeTxt = function (event) {
                if ((attr.qty != undefined && attr.qty !== "false")) {
                    if (!new RegExp("^(-?[0-9]{1,12}(\\.([0-9]{1,11})?)?)?$").test(scope.textControl.val().replace(",", ""))) {
                ...
                if ((attr.creditcard != undefined && attr.creditcard !== "false")) {
                    if (!new RegExp("^(-?[0-9]{1,})?$").test(scope.textControl.val().replace(",", ""))) {
                        scope.fmModel = scope.historyValue;
                    } else {
                        scope.historyValue = scope.fmModel.replace(",", "");
                        scope.fmModel = scope.fmModel.replace(",", "");

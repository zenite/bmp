...
(function (window, document, undefined) {
  ...
  angular.module('mgcrea.ngStrap.tooltip', [ 'mgcrea.ngStrap.core', 'mgcrea.ngStrap.helpers.dimensions' ]).provider('$tooltip', function() {
    ...
    }).directive('maTabs', ['$window', '$animate', '$tab', '$parse', function ($window, $animate, $tab, $parse) {
    var defaults = $tab.defaults;
    return {
            require: ['?ngModel', 'maTabs'],
      transclude: true,
      scope: true,
      ...
      link: function postLink(scope, element, attrs, controllers) {
        ...
        var maTabsCtrl = controllers[1];
        if (ngModelCtrl) {
                    maTabsCtrl.$activePaneChangeListeners.push(function () {
            ngModelCtrl.$setViewValue(maTabsCtrl.$panes.$active);
          });
                    ngModelCtrl.$formatters.push(function (modelValue) {
            maTabsCtrl.$setActive(modelValue);
            return modelValue;
          });
        }
        if (attrs.activePanel) {
            var parsedMaActivePane = $parse(attrs.activePanel);
                    maTabsCtrl.$activePaneChangeListeners.push(function () {
              parsedMaActivePane.assign(scope, maTabsCtrl.$panes.$active);
          });
          scope.$watch(attrs.activePanel, function (newValue, oldValue) {
            maTabsCtrl.$setActive(newValue);
          }, true);
        }
      }
    };
    }]).directive('maPanel', ['$window', '$animate', '$sce', function ($window, $animate, $sce) {
    return {
        require: ['^?ngModel', '^maTabs'],
        scope: true,
        link: function postLink(scope, element, attrs, controllers) {
            ...
            if (maTabsCtrl.$options.animation) {
                element.addClass(maTabsCtrl.$options.animation);
            }
                attrs.$observe('disabled', function (newValue, oldValue) {
                scope.disabled = scope.$eval(newValue);
            });
            maTabsCtrl.$push(scope);
                scope.$on('$destroy', function () {
                maTabsCtrl.$remove(scope);
            });

            function render() {
                var index = maTabsCtrl.$panes.indexOf(scope);
                $animate[maTabsCtrl.$isActive(scope, index) ? 'addClass' : 'removeClass'](element, maTabsCtrl.$options.activeClass);
            }
    ...
    var defaults = this.defaults = {
        ...
        this.$get = ['$window', '$q', '$rootScope', '$bsCompiler', '$animate', '$timeout', '$sce', 'dimensions',
        function ($window, $q, $rootScope, $bsCompiler, $animate, $timeout, $sce, dimensions) {
      ...
      function ModalFactory(config) {
        ...
        $modal.$id = options.id || options.element && options.element.attr('id') || '';
                    forEach(['title', 'content', 'cssType', 'confirmText', 'cancelText'], function (key) {
          if (options[key]) scope[key] = $sce.trustAsHtml(options[key]);
        });
          scope.size = $modal.$options.size;
          scope.textAlign = $modal.$options.textAlign;
          scope.OkTxt = $modal.$options.OkTxt;
        ...
        backdropElement.css({
          ...
          if (options.backdrop) {
            ...
            modalElement.addClass(options.prefixClass);
            backdropElement.css({
          ...
          var el = modalElement[0];
                        requestAnimationFrame(function () {
              el.focus();
              if (el.className.indexOf('aside') !== -1)
                  return;

              //窗体可拖拽 开始
              //1、找到可移动的窗口div
              var moveDiv = el.children[0];
              //2、找到移动DIV的标题（可拖拽移动的部分）
              var titleDiv = moveDiv.querySelector(".modal-header");
              if (!!titleDiv) {
                  titleDiv.style.cursor = "default";
                  //3、基础变量声明（是否开始移动、基础移动坐标）
                                var isStart = false, isDragLeft = false, isDragRight = false, startLocation = { x: 0, y: 0 }, startRightMargin = 0;
                  //4、鼠标按下去开始移动
                  titleDiv.onmousedown = function (e) {
                      startLocation.x = parseInt(moveDiv.style.marginLeft || (moveDiv.parentNode.offsetWidth - titleDiv.offsetWidth) / 2) - (e.clientX || e.x);
                      startLocation.y = parseInt(moveDiv.style.marginTop || 30) - (e.clientY || e.y);
                      isStart = true;
//                      moveDiv.style.userSelect = "none";
                  }
                  el.onmouseup = function (e) {
                      ...
                      isDragRight = false;
                      titleDiv.style.cursor = "default";
//                      moveDiv.style.userSelect = "all";
                  }
                  el.onmousemove = function (e) {
                      if (isStart) {
                          moveDiv.style.marginLeft = (e.clientX || e.x) + startLocation.x + "px";
                          moveDiv.style.marginTop = (e.clientY || e.y) + startLocation.y + "px";
                          titleDiv.style.cursor = "move";
                      }
                     
                      if (isDragLeft) {
                          moveDiv.style.marginLeft = (e.clientX || e.x) + "px";
                          moveDiv.style.width = moveDiv.parentNode.offsetWidth - startRightMargin - parseInt(moveDiv.style.marginLeft) + "px";
                      }
                      if (isDragRight) {
                          if (!moveDiv.style.marginLeft) moveDiv.style.marginLeft = parseInt(moveDiv.style.marginLeft || (moveDiv.parentNode.offsetWidth - titleDiv.offsetWidth) / 2) + "px";
                          moveDiv.style.width = (e.clientX || e.x) - parseInt(moveDiv.style.marginLeft) + "px";
                          startRightMargin = moveDiv.parentNode.offsetWidth - parseInt(moveDiv.style.width) - parseInt(moveDiv.style.marginLeft);
                          console.info(startRightMargin);
                      }
                  }
                  //5、增加鼠标下左右拉伸区域
                  var leftDiv = document.createElement("div");
                  leftDiv.className = "left-drag-div";
                  var rightDiv = document.createElement("div");
                  rightDiv.className = "right-drag-div";
                  //增加向左拖拽事件
                  moveDiv.appendChild(leftDiv);
                  leftDiv.onmousedown = function (e) {
                      isDragLeft = true;
                      if (!startRightMargin) startRightMargin = parseInt(moveDiv.style.marginLeft || (moveDiv.parentNode.offsetWidth - titleDiv.offsetWidth) / 2);
//                      moveDiv.style.userSelect = "none";
                  }
                  moveDiv.appendChild(rightDiv);

...
(function (factory) {
    ...
    angular.module('ng-sortable', ['mabp', 'mabp.orgChart'])
				...
				link: function (scope, $el) {
					...
					sortable = Sortable.create(el, Object.keys(options).reduce(function (opts, name) {
						...
						onSort: function (/**Event*/evt) {
						    if (!!options.orderMember) {
						        var source = getSource();
						        for (var i = 0; i < source.length; i++) {
						            source[i][options.orderMember] = i + 1;
						        }
						    }
							_emitEvent(evt);
						}
					...
					angular.forEach([
						...
						'onStart', 'onEnd', 'onAdd', 'onUpdate', 'onRemove', 'onSort', 'orderMember'
					], function (name) {
                    ...
                    function setRemark() {
                        if (!!options.handle) {
                            if ($(el).find("i.handle").length == 0) {
                                if ($(el).is("tbody")) {
                                    $(el).find("tr > td:not(.ma-lang-label):first-child").append("<i class='handle'></i>");
                                } else {
                                    $(el).children().append("<i class='handle'></i>");
                                }
                            }
                        }
                    }

                    setTimeout(setRemark, 1000);

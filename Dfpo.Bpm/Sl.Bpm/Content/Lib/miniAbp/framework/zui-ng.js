(function () {
    angular.module('mabp')
            ...
            return {
                ...
                transclude: true,
                template: '<div class="col-xs-8" style="text-align: right;">' +
                         '<span style="margin: 15px; line-height: 27px;"><span class="control-label ng-binding" style="vertical-align: top; line-height: 32px;">{{language.EachPageDisplay}}: </span> ' +
                    '<select class="form-control input-sm" style=" width: auto;  display: inline-block;"  ng-model="pageSize"  ng-options="val for val in pageSizeList"> </select></span>' +
                    '<span class="control-label" style="vertical-align: top; line-height: 32px;">' +
                    '{{language.TotalPageCount}}：{{totalCount}} &nbsp;' +
                    '</span> &nbsp;&nbsp;&nbsp;' +
                    '<ul class="pager pull-right" ng-if="totalPage > 0"> <li class="previous" ng-class="{ false : \'disabled\'}[enableStart]"  ng-click="go(currentPage - 1 )"><a>« {{language.PreviousPage}}</a></li> ' +
                    '<li ng-repeat="p in pageIndex"  ng-class="{ \'disabled\' :  currentPage == p}" ng-click="go(p)"><a>{{p}}</a></li>' +
                    '<li class="next"  ng-class="{ false : \'disabled\'}[enableEnd]"  ng-click="go(currentPage +1 )"><a>{{language.NextPage}} »</a></li> ' +
                    '</ul>' +
                    '</div>',
                link: function(scope, element, attr) {
                    ...
                    scope.pageSizeList = [6, 7, 8, 10, 20, 50, 100];
            ...
            return {
                ...
                transclude: true,
                template: '<ul class="pager pull-right"> <li class="previous" ng-class="{ false : \'disabled\'}[enableStart]"  ng-click="go(currentPage - 1 )"><a>« {{language.PreviousPage}}</a></li> ' +
                        '<li ng-repeat="p in pageIndex"  ng-class="{ \'disabled\' :  currentPage == p}" ng-click="go(p)"><a>{{p}}</a></li>' +
                        '<li class="next"  ng-class="{ false : \'disabled\'}[enableEnd]"  ng-click="go(currentPage +1 )"><a>{{language.NextPage}} »</a></li> ' +
                        '</ul>',
                link: function (scope, element, attr) {

...
angular.module("w5c.validator", ["ng"])
    .provider('w5cValidator', [function () {
        var defaultRules = {
            ...
            elemTypes = ['text', 'password', 'email', 'button', 'number', 'url', 'tel', 'hidden', ['textarea'], ['select'], ['select-multiple'], ['select-one'], 'radio', 'checkbox'];
        ...
        var validatorFn = function () {
            ...
            this.defaultShowError = function (elem, errorMessages) {
                ...
                if (!$next || (!$next.hasClass("w5c-error") && !$next.next().hasClass("w5c-error"))) {
            ...
            this.defaultRemoveError = function (elem) {
                ...
                if ($next.hasClass && $next.hasClass("w5c-error")) {
                    //($next.hasClass("w5c-error") || $next.next().hasClass("w5c-error"))
                    $next.remove();
                }
        ...
        validatorFn.prototype = {
            ...
            setDefaultRules: function (rules) {
...
(function () {
    angular.module("w5c.validator")
                    ...
                    this.initElement = function (elem) {
                        ...
                        if (w5cValidator.elemTypes.toString().indexOf(elem.type) > -1 && !w5cValidator.isEmpty(elem.name) && !/^\d/.test(elem.name)) {
                            ...
                            if (disabled && (disabled === 'true' || disabled === 'disabled')) {
                                //校验disabled 属性的 input 值 是否需要required校验
                                if (required) {
                            ...
                            //光标移走的时候触发验证信息
                            if (ctrl.options != null && ctrl.options.blurTrig) {
        ...
        .directive("formSubmit", ['$parse', function ($parse) {
            return {
                ...
                link: function (scope, element, attr, ctrl) {
                    ...
                    ctrl.needBindKeydown = false;
                    ctrl.submitSuccessFn = validSuccessFn;
                }
            };
        }])
        .directive("fmFormSubmit", ['$parse', function ($parse) {
            return {
                require: "^w5cFormValidate",
                link: function (scope, element, attr, ctrl) {
                    var validSuccessFn = $parse(attr.formSubmit);
                    element.bind("click", function (event) {
                        var result = ctrl.doFormValidate();
                        if (result) {
                            scope.$emit('fmFormSubmit', scope.btn);
                        }
                    });
                    ctrl.needBindKeydown = false;
                    ctrl.submitSuccessFn = validSuccessFn;
        ...
        .directive("maFormSubmit", ['$parse', function ($parse) {
            return {
                require: "^w5cFormValidate",
                link: function (scope, element, attr, ctrl) {
                    var validSuccessFn = $parse(attr.maFormSubmit);
                    element.bind("click", function (event) {
                        ctrl.doValidate(validSuccessFn, event);
                    });
                    ctrl.needBindKeydown = true;
        ...
        .directive("maRepeat", [function () {
            'use strict';
            return {
                require: ["ngModel", "^w5cFormValidate"],
                link: function (scope, elem, attrs, ctrls) {
                    var otherInput = elem.inheritedData("$formController")[attrs.maRepeat];
                    var ngModel = ctrls[0], w5cFormCtrl = ctrls[1];
                    ngModel.$parsers.push(function (value) {
                        if (value === otherInput.$viewValue) {
                            ngModel.$setValidity("repeat", true);
        ...
        .directive("maDynamicElement", ["$timeout", function ($timeout) {
            return {
                restrict: 'A',
                ...
                link: function (scope, elm, attrs, ctrls) {
                    ...
                    if (name) {
                        ...
                        if (formCtrl!= null && !formCtrl[name]) {
                            formCtrl.$addControl(ctrls[0]);
                        }
                        var needValidate = false;
                        if (ctrls[2] != null && ctrls[2].$errors && ctrls[2].$errors.length > 0) {
                            needValidate = true;
                        }
                        if(ctrls[1] != null ) ctrls[1].initElement(elm[0]);
                        if (needValidate) {
                            $timeout(function () {
                                if (ctrls[1] != null)
        ...
        .directive('fmDynamicName', [function () {
            return {
                restrict: 'A',
                require: "ngModel",
                link: function (scope, elm, attrs, ngModelCtr) {
                    var name = attrs.placeholder;
                    //ngModelCtr.$name = scope.$eval(attrs.w5cDynamicName);
                    //elm.attr('name', scope.$eval(attrs.w5cDynamicName));
                    //var formController = elm.controller('form') || {
                    //    $addControl: angular.noop

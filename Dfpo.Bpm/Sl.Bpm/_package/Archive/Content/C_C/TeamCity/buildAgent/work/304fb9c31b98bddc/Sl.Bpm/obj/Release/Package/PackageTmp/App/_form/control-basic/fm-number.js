_shared.directive('fmNumber', ['fmTool', '$timeout', function (fmTool, $timeout) {
    return {
        ...
        template: function (elem, attr) {
            ...
            //if (!!attr.scale) {
            //    if (parseInt(attr.scale) == 0) _validate.push("ng-pattern='/^[-]?\\d+$/'");
            //    else _validate.push("ng-pattern='/^(-?\\d+)(\\.\\d{0,6})?$/'");
            //} else {
            //    attr.scale = "0";
            //    _validate.push("ng-pattern='/^[-]?\\d+$/'");
            //}
            if (!attr.name) {
                attr.name = attr.fmModel.toString().substring(attr.fmModel.lastIndexOf(".") + 1);
            }
            return '<div field-name="{{fieldName}}" fm-model="' + attr.fmModel + '" title="{{getShadeText()}}" class="input-sm fm-text form-control ' + (attr.cny != undefined ? 'text-align-right' : '') + '" ng-class="state" ng-mousedown="event_textboxClick($event)" ng-keydown="event_textBoxKeyDown($event)">' +
                        ...
                        '<label ng-hide="!hasShadeText()" class="fm-text-shadetext">{{getShadeText()}}</label>' + //readonly时作为主要显示文本，其他状态下为遮罩层
        ...
        link: function (scope, element, attr) {
            ...
            scope.getShadeText = function () {
                if (!!scope.fmModel) {
                    ...
                    else return parseFloat(scope.fmModel || 0).getPrecision(parseInt(attr.displayScale));
                } else return "";
            }
            //是否有遮罩文本
            ...
            scope.event_changeTxt = function (event) {
                if (!new RegExp(attr.fmPattern).test(scope.textControl.val())) {
                    scope.fmModel = scope.historyValue;
                } else {
                    scope.historyValue = scope.fmModel;
                }
                $timeout(function () {
                    scope.callbackFunc();
                }, 0);

...
function controller(base, form, program) {
    ...
    form.filter = {
        //开始时间
        applyapplicationTimeStart: moment(new Date()).startOf('month').format("YYYY-MM-DD") + " 00:00:00",
        //结束时间
        applyapplicationTimeEnd: moment(new Date()).endOf('month').format("YYYY-MM-DD") + " 23:59:59"
    }
    ...
    form.$watch('form.currentData', function (newV) {
        if (!!newV) {
            ...
            allSnNumber.on('click', function () {
                var id = $(this).attr('id');
                var row = _.find(form.fin13Report, { id: id });
                debugger;

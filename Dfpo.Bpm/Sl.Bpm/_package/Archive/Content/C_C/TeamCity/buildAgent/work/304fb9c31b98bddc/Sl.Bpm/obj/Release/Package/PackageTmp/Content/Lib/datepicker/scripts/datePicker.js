...
Module.constant('datePickerConfig', {
    template: '/Content/Lib/datepicker/templates/datepicker.html',
    view: 'month',
...
Module.filter('mFormat', function () {
    return function (m, format, tz) {
        ...
        if (!(moment.isMoment(m))) {
            if (/^\d{1,2}([:-]|\s)\d{1,2}$/.test(m)) {
                return moment("2000-01-01 " + m).format(format);
            } else {
    ...
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            scope.selectDate = function (date) {
                ...
                if (!nextView && autoclose) {
                    $(element).parent().next().one("click", function () {
                        scope.$emit('showPicker');
                    });
                }
            ...
            setDate = function (date) {
                if (date) {
                    date = date.format(scope.$parent.format);
                    scope.model = date;
                    if (ngModel) {
                ...
                scope.$emit('maSetDate', scope.model, scope.view);
            ...
            scope.$on('clearDate', function (event, newDate) {
                setDate(newDate);
            });

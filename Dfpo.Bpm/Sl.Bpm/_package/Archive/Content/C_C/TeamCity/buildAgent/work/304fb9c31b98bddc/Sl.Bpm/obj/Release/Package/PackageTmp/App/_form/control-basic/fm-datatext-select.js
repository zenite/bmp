_shared
/*下拉列表指令*/
.directive("fmDatatextSelect", ['fmTool', '$timeout', 'dialog', 'mabp.app.module', 'mabp.app.bpm', function (fmTool, $timeout, dialog, service, bpmService) {
    return {
        restrict: 'E',
        replace: true,
        template: function (elem, attr) {
            ...
            return '<div fm-model="' + attr.fmModel + '" fm-text-model="' + attr.fmTextModel + '" class="input-sm ma-datatext-select ma-data-select ' + (attr.ismulti ? 'multi' : 'single') + ' ' + attr.class + '" ng-class="state" ng-mousedown="openSelect($event)" ng-blur="event_Blur()">' +
                        (!!attr.placeholder ? '<div class="placeholder" ng-class="{\'hide\':!!fmModel}">' + attr.placeholder + '</div>' : '') +
                        '<div class="select-data-value">' +
                            '<input placeholder="' + attr.placeholder + '" class="item ' + (attr.href ? 'href-item' : '') + '" type="" ng-readonly="state==\'readonly\'" ng-disabled="state==\'disabled\' || ' + attr.textDisabled + '" ng-model="fmTextModel" ng-change="changeTxt()" />' +
                        '</div>' +
                ...
                post: function postLink(scope, element, attr, cacheCtrl) {
                    ...
                    scope.$watch("filter", function (newvalue, oldvalue) {
                        if (!!scope.filter && scope.state == "normal") {
                            fmTool.setDataSource(scope, attr, service, bpmService, cacheCtrl);
                        }
                    }, true);
                    ...
                    //$timeout(function () {
                    //    if (!!scope.fmModel) {
                    //        fmTool.getOne(scope, attr, service, bpmService);
                    //    }
                    //});
                    var _tempVal = { id: "", text: "" };
                    //修改文本
                    scope.changeTxt = function () {
                        if (!!_tempVal.id) {
                            if (scope.fmTextModel == _tempVal.text) {
                                scope.fmModel = _tempVal.id;
                            } else {
                                scope.fmModel = null;
                            }
                        } else {
                    ...
                    scope.$watch("selectItem", function () {
                        ...
                        if (scope.state == "normal") scope.callbackFunc({ selectItem: scope.selectItem, fmModel: scope.fmModel });
                    }, true);
                    ...
                    scope.event_SelectData = function () {
                        if (scope.state == "normal") {
                            switch (scope.datatype) {
                                case "source":
                                    dialog.open(_shared.dialogs['formSelect'], { data: scope.items, datatype: "source", filter: scope.filter, selectData: scope.fmModel, column: scope.columns, title: attr.placeholder, ismulti: scope.ismulti, valueMember: attr.valueMember, size: attr.size }).then(function (data) {
                                        ...
                                        $timeout(function () {
                                            if (!!scope.selectItem && scope.selectItem.length > 0) scope.fmTextModel = scope.selectItem[0].text;
                                            _tempVal = scope.selectItem[0];
                                        });
                                    });
                                    break;
                                case "view":
                                    dialog.open(_shared.dialogs['formSelect'], { datacode: scope.datacode, datatype: "view", filter: scope.filter, selectData: scope.fmModel, column: scope.columns, title: attr.formTitle, ismulti: scope.ismulti, valueMember: attr.valueMember, size: attr.size }).then(function (data) {
                                        if (data !== null && data !== undefined) {
                                            scope.items = data.items;
                                            scope.fmModel = data.selectItem;
                                            scope.IsSelectData = true;
                                ...
                                case "dataitem":
                                    ...
                                    break;
                                case "group":
                                    dialog.open(_shared.dialogs['treeSelect'], { data: scope.items, datatype: "group", filter: scope.filter, selectData: angular.copy(scope.selectItem), column: scope.columns, title: attr.formTitle, ismulti: scope.ismulti, mustleaf: attr.mustleaf }).then(function (data) {
                                        if (data !== null && data !== undefined) {
                                            scope.items = data.items;
                                            scope.selectItem = data.selectItem;
                                            var ids = [];

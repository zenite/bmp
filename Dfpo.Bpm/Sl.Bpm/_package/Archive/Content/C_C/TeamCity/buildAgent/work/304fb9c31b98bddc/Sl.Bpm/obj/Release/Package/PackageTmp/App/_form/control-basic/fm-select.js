            ...
            template: function (elem, attr) {
                ...
                return '<div fm-model="' + attr.fmModel + '" field-text="{{getShadeText()}}" tabindex="-1" class="input-sm ma-select ' + (attr.ismulti ? 'multi' : 'single') + ' ' + attr.class + '" ng-class="state" ng-mousedown="openSelect($event)" ng-blur="blur()" ng-focus="focus()">' +
                            ...
                            '<div class="select-value">' +
                                '<div class="item" ng-repeat="item in selectItem" ng-mousedown="event_removeSelectItem(item, $event)">{{item.' + attr.displayMember + '}}' + (attr.ismulti ? '' : '<i ng-hide="!iClear" class="icon-times remove" ng-mousedown="event_ClearItem($event)"></i>') + '</div>' +
                            '</div>' +
                            '<div class="option" title="">' +
                                ...
                                '<div class="option-item" ng-mousedown="stopPropagation($event)" >' +
                                    '<ul>' +
                                        '<li ng-mousedown="event_selectItem(item, $event)" ng-class="{\'active\': isCheck(item)}" ng-repeat="item in items | filter: findText track by $index">{{item.' + attr.displayMember + '}}</li>' +
                                    '</ul>' +
                                '</div>' +
                            '</div>' +
                    ...
                    post: function postLink(scope, element, attr, ctrl) {
                        ...
                        $timeout(function () {
                            if (scope.state == "normal") {
                                ...
                                //fmTool.getOne(scope, attr, service);
                            }
                        }, 100);
                                ...
                                if (!scope.items) {
                                    switch (scope.datatype) {
                                        case "source":
                                            break;
                                        case "view":
                                            fmTool.getView(scope.datacode, scope.filter, service, function (data) {
                                                if (!!data) scope.items = data.viewTable;
                                            });
                                            break;
                                        case "dataitem":
                                            fmTool.getBasicdata(scope.datacode, scope.filter, service, function (data) {
